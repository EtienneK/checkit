...
public class BgCoZaExtractor implements Extractor {
  ...
  public Observable<Item> extract() {
    return Observable.from(new String[] {BG_PAGE_URL, BG_EXP_PAGE_URL})
                     .flatMap(url -> {
  ...
  private SuccessCallback<? super ResponseEntity<String>> handleItemListPage(Subscriber<? super Item> subscriber) {
    return httpEntity -> {
      extractItems("div.hikashop_products_listing div.hikashop_products div.hikashop_subcontainer", httpEntity).stream()
                                                                                                               .forEach(
                                                                                                                   subscriber::onNext);
  ...
  private Item map(Element el) {
    String name = el.select("span.hikashop_product_name a")
                    .text();

    BigDecimal price = new BigDecimal(el.select("span.hikashop_product_price")
                                        .text()
                                        .replaceAll(",", ".")
                                        .replaceAll("ZAR", "")
                                        .replaceAll(" ", "")
    ...
    String url = BASE_URL + el.select("span.hikashop_product_name a")
                              .attr("href")
                              .toString();

    StockStatus stockStatus = StockStatus.IN_STOCK_STORE;
    if (!el.select("form div.hikashop_product_no_stock")
           .isEmpty()) {
