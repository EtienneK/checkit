...
public class TimelessExtractor implements Extractor {
	...
	private SuccessCallback<? super ResponseEntity<String>> handleExpansionListPage(
			Subscriber<? super String> subscriber) {
		return httpEntity -> {

			subscriber.onNext(BG_PAGE_URL);
			Jsoup.parse(httpEntity.getBody()).select("div.grid_3 div.box span.thumbnail p strong a").stream()
					.map(el -> BASE_URL + "/" + el.attr("href")).forEach(subscriber::onNext);
	...
	private SuccessCallback<? super ResponseEntity<String>> handleItemListPage(Subscriber<? super Item> subscriber) {
		return httpEntity -> {
			...
			List<Item> items = extractItems("div.grid_3 div.box", httpEntity).stream().collect(Collectors.toList());
